from baselines.siggraph.training_utils import *
import gym
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', help='name of exp', type=str, default='')
    args = parser.parse_args()
    logger.reset()
    logger.configure('data/ppo_' + args.env + str(
        args.seed) + '_walk' + args.name)

    env = gym.make(args.env)
    env.env.assist_timeout = 100.0
    env.env.target_vel = 1.0
    env.env.init_tv = 0.0
    env.env.final_tv = 1.0
    env.env.tv_endtime = 0.5
    env.env.energy_weight = 0.4
